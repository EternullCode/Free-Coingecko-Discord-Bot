...
function getCoingecko() {
  ...
  if (process.env.DISCORD_TOKEN_ID != "" && APIid != "") {
    ...
    console.log("TIP: DISCORD_TOKEN_ID(.env) or APIid(config.json) missing. Press CTRL + C to exit!");
  }
  ...
  bot.login(process.env.DISCORD_TOKEN_ID).catch(error => console.log("TIP: Missing or invalid DISCORD_TOKEN_ID. Check .env file"))
}
